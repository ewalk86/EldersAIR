---
title: 'EldersAIR: ITT Analysis for Health Outcomes'
author: "Ethan Walker"
...
table(model.frame(itt_health_results_primary)$treatment)

nobs(itt_health_results_primary)
...
tidy_results_combined_sbp <- rbind(tidy_results_primary, tidy_results_confounders,
                               tidy_results_nohome) %>% 
  mutate(outcome = "Systolic BP")
...
tidy_results_combined_dbp <- rbind(tidy_results_primary, tidy_results_confounders,
                               tidy_results_nohome) %>% 
  mutate(outcome = "Diastolic BP")
...
table(model.frame(itt_health_results_acceptable)$treatment)
...
tidy_results_combined_fvc <- rbind(tidy_results_primary, tidy_results_acceptable, 
...
fvc_data_2row <- analysis_data %>% 
  #filter(spiro_status == "Acceptable") %>%  # only use this filter for spiro outcomes
  mutate(outcome = fvc_pp) %>%  # select outcome here
  arrange(area, home, adult_id_char, winter_id, sampling_visit) %>% 
  group_by(area, home, adult_id_char, winter_id) %>% 
  mutate(outcome = if_else(winter_id == 1, mean(outcome, na.rm = T), outcome)) %>% 
  dplyr::select(area, home, adult_id_char, winter_id, sampling_visit, 
...
itt_plot_estimates <- tidy_results_combined %>%
  ...
  ggplot(aes(group = term, shape = term)) +
    ...
    geom_hline(yintercept = 0) +
    theme_bw() +  
    ggtitle(label = "FVC % Pred Model - ITT framework",
            subtitle = "ICC for subject = 0.86") +
    labs(y = "Estimate compared to Placebo") +
    labs(x = "", group = "", shape = "") +
    theme(title = element_text(size = 16), 
...
nobs(itt_health_results_acceptable)
...
tidy_results_combined_fev1 <- rbind(tidy_results_primary, tidy_results_acceptable, 
                               tidy_results_confounders,
                               tidy_results_nohome) %>% 
  mutate(outcome = "FEV1")
...
fev1_data_2row <- analysis_data %>% 
  #filter(spiro_status == "Acceptable") %>%  # only use this filter for spiro outcomes
  mutate(outcome = fev1_pp) %>%  # select outcome here
  arrange(area, home, adult_id_char, winter_id, sampling_visit) %>% 
  group_by(area, home, adult_id_char, winter_id) %>% 
  mutate(outcome = if_else(winter_id == 1, mean(outcome, na.rm = T), outcome)) %>% 
  dplyr::select(area, home, adult_id_char, winter_id, sampling_visit, 
...
itt_plot_estimates <- tidy_results_combined %>%
  ...
  ggplot(aes(group = term, shape = term)) +
    ...
    geom_hline(yintercept = 0) +
    theme_bw() +  
    ggtitle(label = "FEV1 % Pred Model - ITT framework",
            subtitle = "ICC for subject = 0.91") +
    labs(y = "Estimate compared to Placebo") +
    labs(x = "", group = "", shape = "") +
    theme(title = element_text(size = 16), 
...
fev1_data_2row <- analysis_data %>% 
  #filter(spiro_status == "Acceptable") %>%  # only use this filter for spiro outcomes
  mutate(outcome = fev1_fvc_ratio) %>%  # select outcome here
  arrange(area, home, adult_id_char, winter_id, sampling_visit) %>% 
  group_by(area, home, adult_id_char, winter_id) %>% 
  mutate(outcome = if_else(winter_id == 1, mean(outcome, na.rm = T), outcome)) %>% 
  dplyr::select(area, home, adult_id_char, winter_id, sampling_visit, 
...
tidy_results_combined_fev1_fvc_ratio <- rbind(tidy_results_primary, tidy_results_acceptable, 
                               tidy_results_confounders,
                               tidy_results_nohome) %>% 
  mutate(outcome = "FEV1/FVC Ratio")
...
itt_plot_estimates <- tidy_results_combined %>%
  ...
  ggplot(aes(group = term, shape = term)) +
    ...
    geom_hline(yintercept = 0) +
    theme_bw() +  
    ggtitle(label = "FEV1/FVC Ratio Model - ITT framework",
            subtitle = "ICC for subject = 0.91") +
    labs(y = "Estimate compared to Placebo") +
    labs(x = "", group = "", shape = "") +
    theme(title = element_text(size = 16), 
...
Manuscript figure
```{r}
tidy_results_combined <- rbind(tidy_results_combined_sbp, tidy_results_combined_dbp,
                               tidy_results_combined_fvc, tidy_results_combined_fev1,
                               tidy_results_combined_fev1_fvc_ratio)
...
itt_plot_estimates <- tidy_results_combined %>%
  ...
  mutate(outcome = factor(outcome,
                        ...
                        labels = c("Systolic BP (mmHg)", "Diastolic BP (mmHg)",
                                   "FVC (mL)", "FEV1 (mL)", "FEV1/FVC Ratio"))) %>% 
  ggplot(aes(group = term, shape = term)) +
    geom_point(aes(x=model, y=estimate), 
               position = position_dodge(width = 0.5), size = 4) +
    geom_errorbar(aes(x=model, ymin=conf.low, ymax=conf.high), 
...
ggsave("elders_health_itt_plot.png", height = 9, width = 6)
```
