...
```{r}
nn_athomelog_initial <- read_xlsx(paste0(file_path, "Input/NN/nn_athome_20200225.xlsx")) %>% 
...
np_athomelog_initial <- read_xlsx(paste0(file_path, "Input/NP/np_athomelog_20200225.xlsx")) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
         adult_id_num = as.character(AdultID),
         area = "NP",
         sampling_visit = as.numeric(SamplingVisit),
...
# Load and merge files from different locations
```{r}
nn_healthhistory_initial <- read_xlsx(paste0(file_path, "Input/NN/nn_healthhistory_20200225.xlsx")) %>% 
...
np_healthhistory_initial <- read_xlsx(paste0(file_path, "Input/NP/np_healthhistory_20200225.xlsx")) %>% 
  rename_all(~str_replace_all(., "Date", "_date")) %>% 
  rename_all(tolower) %>% 
  mutate(home_winter_id = as.character(homewinterid),
         adult_id_num = as.character(adultid),
...
# Load and merge files from different locations
```{r}
nn_healthextra_initial <- read_xlsx(paste0(file_path, "Input/NN/nn_healthextra_20200225.xlsx")) %>% 
  rename_all(tolower) %>% 
  mutate(home_winter_id = as.character(homewinterid),
         adult_id_num = as.character(adultid),
         area = "NN",
...
np_healthextra_initial <- read_xlsx(paste0(file_path, "Input/NP/np_healthextra_20200225.xlsx")) %>% 
  rename_all(tolower) %>% 
  mutate(home_winter_id = as.character(homewinterid),
         adult_id_num = as.character(adultid),
         area = "NP",
