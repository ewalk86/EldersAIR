...
author: "Ethan Walker"
date: "Started 10 Feb 2020, Updated 10 Feb 2020"
output: pdf_document
---
...
np_adult_initial <- read_csv("Input/NP/np_adults_20200210.csv") %>% 
  rename_all(tolower) %>% 
  mutate(adult_hwid = as.factor(adulthwid),
         home_id = as.factor(homeid),
         adult_id = as.factor(adultid),
         home_winter_id = as.factor(homewinterid)) %>% 
  select(adult_hwid, home_id, adult_id, home_winter_id) 


## Box location: Update Box location once final data loaded
np_home_initial <- read_csv("Input/NP/np_home_20200210.csv") %>% 
  rename_all(tolower) %>% 
