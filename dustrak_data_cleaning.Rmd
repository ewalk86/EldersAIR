...
np_dustrak_initial <- read_xlsx(paste0(file_path, "Input/NP/np_dustrak_20200220.xlsx")) %>% 
  # rename variables
...
#write_rds(np_dustrak_initial, paste0(file_path, "Output/np_dustrak.rds"))


## Box location: Update Box location once final data loaded
nn_dustrak_initial <- read_xlsx(paste0(file_path, "Input/NN/nn_dustrak_20200220.xlsx")) %>% 
  # rename variables
  separate(StartTime, c("trash1", "start_time_pm"), sep = " ", remove = FALSE) %>% 
  separate(StopTime, c("trash1", "stop_time_pm"), sep = " ", remove = FALSE) %>% 
  mutate(home_winter_id = as.factor(HomeWinterID),
  ...
  select(home_winter_id, area, dtid, sampling_visit, start_date_pm, start_time_pm,
         ...
         sample_max, pm_comments) %>% 
  arrange(home_winter_id)
#write_rds(nn_dustrak_initial, paste0(file_path, "Output/nn_dustrak.rds"))


elders_dustrak_log <- rbind(np_dustrak_initial, nn_dustrak_initial)
#write_rds(elders_dustrak_log, paste0(file_path, "Output/elders_dustrak_log.rds"))
...
### NN ###
# List files
list_text_files <- list.files(paste0(file_path, "Input/NN/nn_dustrak/pre_20200218/text"))

# Set working directory and load files in list; extract file name and add as column
## run next 4 lines together
setwd(paste0(file_path, "Input/NN/nn_dustrak/pre_20200218/text")) # set new working directory
...
### NP ###
# List files
list_text_files <- list.files(paste0(file_path, "Input/NP/np_dustrak/pre_20200218/text"))

# Set working directory and load files in list; extract file name and add as column
## run next 4 lines together
setwd(paste0(file_path, "Input/NP/np_dustrak/pre_20200218/text")) # set new working directory
initial_data = tibble(file_name = list_text_files) %>% # use list of files from above
...
### NN ###
# List files
list_csv_files <- list.files(paste0(file_path, "Input/NN/nn_dustrak/pre_20200218/csv"))

# Set working directory and load files in list; extract file name and add as column
## run next 4 lines together
setwd(paste0(file_path, "Input/NN/nn_dustrak/pre_20200218/csv")) # set new working directory
...
### NP ###
# List files
list_csv_files <- list.files(paste0(file_path, "Input/NP/np_dustrak/pre_20200218/csv"))

# Set working directory and load files in list; extract file name and add as column
## run next 4 lines together
setwd(paste0(file_path, "Input/NP/np_dustrak/pre_20200218/csv")) # set new working directory
initial_data = tibble(file_name = list_csv_files) %>% # use list of files from above
  mutate(save_data = lapply(file_name, read_csv)) %>%
  unnest(save_data)
