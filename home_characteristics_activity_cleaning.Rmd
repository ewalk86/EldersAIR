...
np_homechar_initial <- read_xlsx(paste0(file_path, "Input/NP/np_homechar_20200225.xlsx")) %>% 
  # rename variables
...
## Box location: Update Box location once final data loaded
nn_homechar_initial <- read_xlsx(paste0(file_path, "Input/NN/nn_homechar_20200225.xlsx")) %>% 
  # rename variables
  mutate(home_winter_id = as.factor(HomeWinterID),
         residents_total = as.numeric(Residents),
         residents_over55 = as.numeric(Over55),
...
#write_rds(elders_home_char, paste0(file_path, "Output/elders_home_char.rds"))
```


# Clean up all home characteristics data
```{r}
elders_home_char <- read_rds(paste0(file_path, "Output/elders_home_char.rds"))
...
elders_home_char_clean <- elders_home_char %>% 
  ...
  mutate(home_year_built = if_else(home_year_built<100, 0, home_year_built)) %>% 
...
```{r}
## Box location: Update Box location once final data loaded
np_homeactivity_initial <- read_xlsx(paste0(file_path, "Input/NP/np_homeactivity_20200225.xlsx")) %>% 
  # rename variables
  mutate(home_winter_id = as.factor(HomeWinterID),
...
## Box location: Update Box location once final data loaded
nn_homeactivity_initial <- read_xlsx(paste0(file_path, "Input/NN/nn_homeactivity_20200225.xlsx")) %>% 
  # rename variables
  mutate(home_winter_id = as.factor(HomeWinterID),
         home_act_day = as.factor(Day),
         home_act_gas = as.factor(Gas),
...
#write_rds(elders_home_activity, paste0(file_path, "Output/elders_home_activity.rds"))
```


# Clean up all home activity data
```{r}
elders_home_activity <- read_rds(paste0(file_path, "Output/elders_home_activity.rds"))
