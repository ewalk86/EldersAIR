...
np_micropem_initial <- read_xlsx(paste0(file_path, "Input/NP/np_micropem_20200225.xlsx")) %>% 
  # rename variables
  ...
  mutate(home_winter_id = as.character(HomeWinterID),
         ...
         mp_file_name = as.character(mFile),
         fb_file_name = as.character(fbFile),
...
## Box location: Update Box location once final data loaded
nn_micropem_initial <- read_xlsx(paste0(file_path, "Input/NN/nn_micropem_20200225.xlsx")) %>% 
  # rename variables
  separate(StartTime, c("trash1", "start_time_pm"), sep = " ", remove = FALSE) %>% 
  separate(StopTime, c("trash1", "stop_time_pm"), sep = " ", remove = FALSE) %>% 
  mutate(home_winter_id = as.character(HomeWinterID),
...
### NP ###
# List files
list_files <- list.files(paste0(file_path, "Input/NP/np_micropem_20200302"))

# Set working directory and load files in list; extract file name and add as column
## run next lines together
setwd(paste0(file_path, "Input/NP/np_micropem_20200302")) # set new working directory
...
elders_micropem <- read_rds(paste0(file_path, "Output/elders_micropem.rds"))
