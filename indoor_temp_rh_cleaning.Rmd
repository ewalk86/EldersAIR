...
author: "Ethan Walker"
date: "Started 14 Feb 2020, Updated 24 Aug 2020"
output: pdf_document
---
...
np_temp_rh_initial <- read_xlsx(paste0(file_path, "Input/NP/qtrack.xlsx"),
                                na = c("", "NULL")) %>% 
  # rename variables
...
elders_temp_rh_clean <- elders_temp_rh %>% 
  ...
  filter(!is.na(winter_id)) %>% 
  group_by(home, winter_id, sampling_visit) %>% 
  mutate(temp_indoor_max_new = if_else(temp_indoor_max < temp_indoor_min, 
         ...
         rh_indoor_min_new = if_else(rh_indoor_min > rh_indoor_max, 
                                     rh_indoor_max, rh_indoor_min)) %>% 
  ungroup() %>% 
  select(area, home, home_winter_id, home_id_num, winter_id, sampling_visit,
         treatment, temp_indoor_min_new, temp_indoor_max_new,
